...
title: Running molecular dynamics simulations of a protein-ligand system using GROMACS
zenodo_link: ''
questions:
...
In previous tutorials, we performed simulations on the lysozyme protein to demonstrate the basic capabilities of the GROMACS tools in Galaxy. Often, we are interested in a more complex situation where a protein is interacting with another molecule, e.g. a ligand or another protein.

In this tutorial, we will perform a simulation of the cellulase enzyme in complex with a substrate. This time, we will perform the initial steps (topology preparation, solvation) using CHARMM-GUI, then performing minimization, equilibration and production simulations using GROMACS.

This tutorial assumes you have familiarized yourself with the basic [GROMACS tutorial](../md-simulations-gromacs/tutorial.html) and that you have performed the initial setup using CHARMM-GUI as described [here](../setting-up-molecular-systems/tutorial.html).
